function Navbar({ isLoggedIn, onOpenLogin, onLogout, currentView, setCurrentView }) {
  
  // Quick dynamic text styling mapper function
  const linkClass = (view) => 
    `font-semibold text-sm px-1 py-5 transition-all border-b-2 ${
      currentView === view 
    ...
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm w-full">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          {/* Core App View Toggles (Visible after verification) */}
          {isLoggedIn && (

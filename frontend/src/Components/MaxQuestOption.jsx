...
function MaxQuestOption({ onSelectType, onBack }) {
  const options = [
    ...
    { id: "one_liner", title: "One Liner Questions", icon: "✏️", desc: "Short precise direct answer point benchmarks." },
    { id: "long_question", title: "Long Questions", icon: "📖", desc: "Detailed breakdown descriptive assessment topics." }
  ];
  ...
  return (
    <div className="w-full max-w-4xl px-4 py-8 mx-auto animate-in fade-in duration-200">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm mt-1">Pick the specific structure layout for your mass question build pipeline.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {options.map((opt) => (
          ...
          >
            <span className="text-3xl p-3 bg-slate-50 rounded-xl group-hover:bg-indigo-50 transition-colors">{opt.icon}</span>
            <div>
              <h4 className="font-bold text-slate-800 mb-1 group-hover:text-indigo-600 transition-colors">{opt.title}</h4>
